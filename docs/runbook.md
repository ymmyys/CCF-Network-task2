...
```text
yijq27-vllm-qwen7b-3  -> NPU 3, port 9121
yijq27-vllm-qwen7b-4  -> NPU 4, port 9122
yijq27-vllm-qwen7b-5  -> NPU 5, port 9126
yijq27-vllm-qwen7b-6  -> NPU 6, port 9127
yijq27-vllm-qwen7b-7  -> NPU 7, port 9128
```

这些 7B 容器只为 exp8 启动，实验结束后应删除，避免长期占用 NPU。
...
/home/yijq27/workspace/models/Qwen2.5-7B-Instruct
```
...
docker run --rm --entrypoint bash \
  ...
  -v /home/yijq27/workspace/models:/models \
  quay.io/ascend/vllm-ascend:v0.18.0rc1 \
  -lc 'hf download Qwen/Qwen2.5-7B-Instruct \
...
exp8 需要先启动 5 个 Qwen2.5-7B 后端。若存在同名旧容器，只删除这些精确命名的临时容器：
...
```bash
MODEL_DIR=/home/yijq27/workspace/models/Qwen2.5-7B-Instruct
IMAGE=quay.io/ascend/vllm-ascend:v0.18.0rc1

for spec in 3:9121 4:9122 5:9126 6:9127 7:9128; do
  NPU=${spec%:*}
  PORT=${spec#*:}
  docker run -itd \
    --name yijq27-vllm-qwen7b-${NPU} \
    --privileged \
    --net=host \
    --ipc=host \
    --device /dev/davinci${NPU} \
...
```bash
for p in 9121 9122 9126 9127 9128; do
  until curl -fsS http://127.0.0.1:$p/health >/dev/null; do
    echo "waiting $p"
    sleep 5
  done
done
...
| exp8 | `run_experiment8_qwen7b_real.sh` | Qwen2.5-7B 热点避让泛化，使用临时 qwen7b 容器 |
...
```text
config/router.qwen7b-5backends-swrr.json
config/router.qwen7b-5backends-p2c.json
config/router.qwen7b-5backends-balanced.json
```
